...
function KakaoModal({ members, onClose }: KakaoModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: '#00000080' }}>
      <div className="w-full max-w-lg rounded-2xl p-6" style={{ backgroundColor: '#1E2130', border: '1px solid #2A2D3E', maxHeight: '80vh', overflowY: 'auto' }}>
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-lg font-semibold text-white">카카오 메시지 템플릿</h2>
            <p className="text-xs mt-0.5" style={{ color: '#8B8FA8' }}>재등록 확률 70% 이상 회원 ({highProb.length}명)</p>
          </div>
        ...
        <div className="space-y-3">
          {highProb.map(m => {
            ...
            return (
              <div key={m.id} className="rounded-xl p-4" style={{ backgroundColor: '#1A1D27', border: '1px solid #2A2D3E' }}>
                ...
                <pre className="text-xs whitespace-pre-wrap mb-3" style={{ color: '#8B8FA8', fontFamily: 'inherit' }}>{text}</pre>
                <button
                  onClick={() => handleCopy(m.id, text)}
                  className="w-full py-1.5 rounded-lg text-xs font-medium transition-all"
                  style={{
                    backgroundColor: copied === m.id ? '#00E5A020' : '#2A2D3E',
          ...
          {highProb.length === 0 && (
            <p className="text-center py-8 text-sm" style={{ color: '#8B8FA8' }}>재등록 확률 70% 이상 회원이 없습니다.</p>
          )}
        </div>
      </div>
    </div>
...
export default function RenewalBoard() {
  ...
  const weekCounts = members.reduce((acc, m) => {
    const week = getWeekOfMonth(m.expire_date)
  ...
  const avgProb = members.length > 0
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6">
        <WeekTabs activeTab={activeTab} onTabChange={setActiveTab} counts={weekCounts} />
      </div>

      {loading ? (
        <div className="flex items-center justify-center py-20">
          <div className="w-8 h-8 border-2 rounded-full animate-spin" style={{ borderColor: '#2A2D3E', borderTopColor: '#00E5A0' }}></div>
        </div>
      ) : filteredMembers.length === 0 ? (
        <div className="text-center py-20">
          <p style={{ color: '#8B8FA8' }}>이번 달 만료 예정 회원이 없습니다.</p>
          <button
            onClick={() => setShowAddModal(true)}
